 ...
 */
class SyncIO<out A> private constructor(val toIO: IO<A>) {
     ...
     */
    fun unsafeRunSync(): A = toIO.unsafeRunSync()
     ...
     */
    fun <B> map(f: (A) -> B): SyncIO<B> = SyncIO(toIO.map(f))
     ...
     * sequenced in to the result `SyncIO[B].
     */
    fun <B> flatMap(f: (A) -> SyncIO<B>): SyncIO<B> = SyncIO(toIO.flatMap { a -> f(a).toIO })
     ...
     */
    fun attempt(): SyncIO<Either<Throwable, A>> = SyncIO(toIO.attempt())

    /**
     * Converts the source `IO` into any `F` type that implements
     * the [[LiftIO]] type class.
     */
    fun <F> to(LF: LiftIO<F>): Kind<F, A> = LF.liftIO(toIO)
...
private [effect] abstract class SyncIOInstances extends SyncIOLowPriorityInstances {
    implicit
    val syncIoSync: Sync[SyncIO] = new Sync[SyncIO] with StackSafeMonad[SyncIO] {
    ...
    implicit
    val syncIoSemigroupK: SemigroupK[SyncIO] = new SemigroupK[SyncIO] {
...
private [effect] abstract class SyncIOLowPriorityInstances {
    private [effect]
    class SyncIOSemigroup[A: Semigroup] extends Semigroup[SyncIO[A]]
    {
