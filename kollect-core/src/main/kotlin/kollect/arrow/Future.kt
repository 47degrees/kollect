...
interface Future<T> : Awaitable<T> {
     ...
     */
    fun failed(executor: ExecutionContext): Future<Throwable> = transform(internalExecutor) {
        when (it) {
     ...
     * @group Transformations
     */
    fun <S, T : Future<S>> flatten(ev: T): Future<S> = flatMap(internalExecutor, ev)
     ...
     * @group Transformations
     */
    fun filter(executor: ExecutionContext, p: (T) -> Boolean): Future<T> = map { r ->
     ...
     * @group Transformations
     */
    fun <S> collect(executor: ExecutionContext, pf: PartialFunction<T, S>): Future<S> = map { r ->
    ...
    object Future {
        ...
        { value => foldNext(i, op(prevValue, value), op) }
...
// a side effect.
    private [concurrent]
    object InternalCallbackExecutor extends ExecutionContext with BatchingExecutor
    {
